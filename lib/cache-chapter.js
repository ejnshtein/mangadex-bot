...
let cachingChapters = 0

const caching = new Map()

let updatingChapters = 0
...
module.exports = (chapter, manga, me) =>
  new Promise(async (resolve, reject) => {
    ...
    if (cachedChapter) {
      ...
      const uploadedChapter = await uploadChapter('caching', chapter, manga, me)
      return resolve(uploadedChapter)
    }
  })

module.exports.cachePoolSize = () => cachingChapters
module.exports.updatePoolSize = () => updatingChapters
...
const uploadChapter = (mode, chapter, manga, me) => new Promise(async resolve => {
  ...
  for (const img of chapter.page_array) {
    ...
    const timer = setTimeout(() => {
      ended = true
      chapterData.emit('error', new Error(`Caching one picture (${chapterData.cached + 1}) timeout`))
    }, 1000 * 60 * 5)
    try {
  ...
  client.sendMessage(
    ...
    `Manga: <a href="https://mangadex.org/title/${manga.id}">${manga.title}</a>
Chapter: https://mangadex.org/chapter/${chapter.id}
Telegraph: ${telegraph.url}`
  )
  chapterData.emit('done')
  chapterData.emit('endCaching')
  block.delete(chapter.id)
})
