...
module.exports = async (chapter, manga, ctx) => {
  ...
  if (cachedData) {
    if (cachedData.timestamp === chapter.timestamp) {
      ...
      return new Promise(async (resolve) => {
        ...
        for (const img of chapter.page_array) {
          ...
          const file = await uploadFile({ name: img, file: picEdited ? picBuff : buffer })
          // console.log(file)
    ...
    const telegraphPage = await cacheFile.telegraph(chapter, manga, pics, ctx.me)
    // console.log(telegraphPage)
    await collection('chapters').create({
      ...
      manga_title: (await getManga(chapter.manga_id, false)).manga.title
    })
