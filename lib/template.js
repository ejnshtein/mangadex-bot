...
const templates = {
  ...
  date: (date = new Date()) => `${date.toISOString().replace(/-/ig, '.').replace('T', ' ').slice(0, 23)}`,
  manga: {
    view (mangaId, manga, url, withChapters, list) {
      let messageText = `${list ? `${list}\n\n` : ''}<a href="${manga.cover_url}">&#8203;</a><a href="https://mangadex.org/title/${mangaId}">${decode(manga.title)}</a>\n\n`
      if (!withChapters) {
      ...
      messageText += `<b>Genres:</b> ${manga.genres.map(genre => `<a href="https://mangadex.org/genre/${genre.id}">${genre.label}</a>`).join(', ')}\n`
    ...
    chapter (chapter, manga = null, message = null, list) {
      let messageText = `${list ? `${list}\n\n` : ''}${chapter.telegraph ? `<a href="${chapter.telegraph}">&#8203;</a>` : ''}<a href="https://mangadex.org/chapter/${chapter.id}">Read on Mangadex</a>\n`
      messageText += `${decode(manga.title)} ${chapter.volume ? `Vol. ${chapter.volume} ` : ''}Ch. ${chapter.chapter}`
      messageText += `${chapter.title ? `\n\n<b>Chapter title:</b> ${chapter.title}` : ''}\n${message ? storeHistory(message) : '<a href="https://mangadex.org/search?title=">&#8203;</a>'}`
      return messageText
    },
    inlineMangaView (mangaId, manga) {
      let messageText = `<a href="${manga.cover_url}">&#8203;</a><b>Title:</b> <a href="https://mangadex.org/title/${mangaId}">${decode(manga.title)}</a>\n\n`
      messageText += `<b>Genres:</b> ${manga.genres.map(genre => `<a href="https://mangadex.org/genre/${genre.id}">${genre.label}</a>`).join(', ')}\n`
      messageText += `<b>Pub. status:</b> ${manga.status_text}\n`
      return messageText
    },
    inlineQuery (manga) {
      let messageText = `<b>Title:</b> <a href="https://mangadex.org/title/${manga.id}">${decode(manga.title)}</a>\n\n`
      messageText += `<b>Rating:</b> ${manga.rating.value}\n`
      messageText += `<b>Views:</b> ${manga.views}`
      return messageText
    },
    viewPublic (mangaId, manga) {
      let messageText = `<b>Title:</b> <a href="https://mangadex.org/title/${mangaId}">${decode(manga.title)}</a>`
      return messageText
    },
  ...
  chapter: {
    formatChapter (chapter) {
      return `${chapter.volume ? `Vol. ${chapter.volume} ` : ''}Ch. ${chapter.chapter}`
    },
    channel (chapter, manga, telegraph) {
      let messageText = `<a href="${telegraph}">&#8203;</a>`
      messageText += `<a href="https://mangadex.org/title/${chapter.manga_id}">${manga.title}</a> <a href="https://mangadex.org/chapter/${chapter.id}">${templates.chapter.formatChapter(chapter)} in ${loadLangCode(chapter.lang_code)}</a> has been added to database!`
      return messageText
    }
  },
  error (e) {
