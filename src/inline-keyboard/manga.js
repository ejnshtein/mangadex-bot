...
export default async function mangaView ({
  mangaId,
  fromId
}) {
  ...
  const mangaExistsInDb = await collection('mangas')
    .findOne({ id: mangaId })
  if (!mangaExistsInDb) {
  ...
  const result = await collection('mangas')
    .aggregate(
      [
  ...
  for (const [langCode, codeChapters] of Object.entries(chapters)) {
    ...
    const obj = {
      text: buttonText,
      callback_data: `chapterlist:${qs.stringify({ lang: langCode, id: mangaId })}`
    }
  ...
  keyboard.unshift(
    [
      {
        text: mangaInDb.is_favorited ? buttons.favorite(false) : buttons.favorite(),
